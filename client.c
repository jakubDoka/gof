...
void map_server_send(map_t *m) {
    ...
    client_t c;
    client_new_result_t result_new = client_new(&c, "frios2.fri.uniza.sk", 10022);
...
map_t map_server_get(map_t *m) {
    ...
    while (ch != '\n') {
        ...
        switch (ch) {
            ...
            case 'S':
            case KEY_DOWN:
                y += y == res.data.list_worlds.count - 1 ? 0 : 1;
                break;
        }
    }

    r = (request_t) { PROTO_LOAD_WORLD, {.load_world = { res.data.list_worlds.names[y] }}};
    response_free(res);
